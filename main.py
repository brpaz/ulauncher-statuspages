...
class StatusPagesExtension(Extension):
    ...
    def load_pages(self):
        ...
        if (os.path.isfile(os.path.join(EXT_PREFERENCES_DIR, 'status_pages.json'))):
...
class KeywordQueryEventListener(EventListener):
    ...
    def on_event(self, event, extension):
        ...
        if query:
            pages = [p for p in pages if query.lower()
                     in p['label'].lower()]
        ...
        items = []
        for page in pages[:8]:
            items.append(ExtensionResultItem(icon=page['icon'],
                                             name=page['label'],
